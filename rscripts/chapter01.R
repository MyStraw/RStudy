...
# R Session 보기 => R 개발환경 동료에게 알려줄때 많이 사용.
sessionInfo()
...
# 실습: 기본 데이터 셋 보기. 통계학자가 쓴다. 데이터가 있어야돼.
data()
...
# 실습: 히스토그램을 파일로 저장하기 
par(mfrow = c(1, 1))
pdf("batch.pdf") # 학자위한거. 논문에 그림.벡트드로잉 방식. png에 투명도 높은거도
hist(rnorm(20))
dev.off() #이걸로 써서 저장. 경로 없애니까 우리 데이터 루트에 저장. c:batch.pdf 여기서 앞에 지움
...
sum(10, 20, 20)
sum(10, 20, 20, NA)
sum(10, 20, 20, NA, na.rm = TRUE) #NA - 숫자아냐.
#na.rm na가 비어있으면 넘어가. 판다스에서 s. 합계
#합산에는 NA에 0을 넣으면 되는데 평균에는 쓰이면 안돼
...
# 현재 작업 공간 보기(기본함수) 현재 어느위치에서 하는지 나옴
getwd()
