...
#https://github.com/sigmadream/practice-r
install.packages("remotes") #github 설치용
remotes::install_github("bit2r/bitTA") #깃허브(ID/저장소명), #한국어 형태소분석
library(bitTA) #라이브러리 호출, import
bitTA::install_mecab_ko() #한국어 형태소 사전 및 형태소 분석기를 다운로드
install.packages("RcppMeCab")

library(bitTA)
library(tidyverse) 
library(tidytext) #한국어 텍스트 쓸만한거? 대통령 담화문.
#NLP 3가지 설치 끝. 
...
president_speech$doc[1:100] %>% #대통령 말씀자료 1번부터 100번까지 모으면.
  morpho_mecab(indiv = FALSE) %>% 
  table() %>% #명사를 상정시키다. 표로 올린다. 키 밸류. count 쓰면 A라는 단어가 몇번 나왔는지 세는거
  #명
  wordcloud2::wordcloud2() #워드클라우드에 넣어봐.
...
# 실습: 소설을 활용한 감정 분석 #시 같은거 갖고오지마 #분석가의 편향
